...
class Conf:
    ...
    The configuration can be accessed via the `options` property, which returns an instance of
    `ChromeOptions` configured as per the set attributes.
    ...
    def options(self) -> ChromeOptions:
        ...
        This property method initializes a `ChromeOptions` instance and configures it based on
        the class attributes like headless mode, incognito mode, etc.
